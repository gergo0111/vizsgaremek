...
export function GanntChart() {
    ...
    const [filters, setFilters] = useState<Record<StatusKey, boolean>>({
        p0_25: true, p25_50: true, p50_75: true, p75_100: true,
        late: true, done: true, planned: true,
    });
    ...
    // Nézet tartományának kiszámítása
    ...
    // Szűrés és rendezés - Itt dől el, mi látszik a listában és a rácson
    const filteredSorted = useMemo<DecoratedWork[]>(() => {
        const visible = decoratedAll.filter((w) => {
            // 1. Státusz szűrő (checkboxok)
            const isStatusMatch = filters[w.status];
            // 2. Időbeli szűrő: van-e átfedés a munka és a nézet között?
        ...
        return [...visible].sort((a, b) => {
            switch (sortKey) {
                case "name": return a.munka_neve.localeCompare(b.munka_neve) * dir;
                case "start": return (a.start.getTime() - b.start.getTime()) * dir;
                case "end": return (a.end.getTime() - b.end.getTime()) * dir;
    ...
    return (
        <div className="pt-page">
                ...
                <div className="pt-panel pt-left">
                    ...
                    <div className="pt-sortBar">
                        <div className="pt-sortGroup">
                            <button className={`pt-sortBtn ${sortKey === "name" && sortDir === "asc" ? "pt-sortBtn--active" : ""}`} onClick={() => { setSortKey("name"); setSortDir("asc"); }}>Név ▲</button>
                            <button className={`pt-sortBtn ${sortKey === "name" && sortDir === "desc" ? "pt-sortBtn--active" : ""}`} onClick={() => { setSortKey("name"); setSortDir("desc"); }}>Név ▼</button>
                        </div>
                        <div className="pt-sortGroup">
                            <button className={`pt-sortBtn ${sortKey === "start" && sortDir === "asc" ? "pt-sortBtn--active" : ""}`} onClick={() => { setSortKey("start"); setSortDir("asc"); }}>Kezdet ▲</button>
                            <button className={`pt-sortBtn ${sortKey === "start" && sortDir === "desc" ? "pt-sortBtn--active" : ""}`} onClick={() => { setSortKey("start"); setSortDir("desc"); }}>Kezdet ▼</button>
                        </div>
                        <div className="pt-sortGroup">
                            <button className={`pt-sortBtn ${sortKey === "end" && sortDir === "asc" ? "pt-sortBtn--active" : ""}`} onClick={() => { setSortKey("end"); setSortDir("asc"); }}>Határidő ▲</button>
                            <button className={`pt-sortBtn ${sortKey === "end" && sortDir === "desc" ? "pt-sortBtn--active" : ""}`} onClick={() => { setSortKey("end"); setSortDir("desc"); }}>Határidő ▼</button>
                        </div>
                    </div>
                    ...
                    <div className="pt-leftList">
                        ...
                        {filteredSorted.map((w) => (
                            <div key={w.munka_id} className="pt-workCard">
                                <div className={`pt-bar ${progressColorClass(w.progress)} ${w.status === "late" ? "pt-bar--late" : ""}`}>
                                    <div className="pt-barFill" style={{ width: `${w.progress}%` }} />
                                    <div className="pt-barLabel">{w.munka_neve} {w.progress}%</div>
                                </div>
                            </div>
                        ))}
                    </div>
                ...
                <div className="pt-panel pt-right" style={{ ["--pt-days" as any]: days.length }}>
                    <div className="pt-ganttHeader">
                        <div className="pt-ganttYearNav">
                            <button className="pt-navBtn" onClick={() => step(-1)}>◀</button>
                            <div className="pt-year">{rangeStart.getFullYear()}</div>
                            <button className="pt-navBtn" onClick={() => step(1)}>▶</button>
                        </div>
                        <div className="pt-filter">
                            <select className="pt-viewSelect" value={viewMode} onChange={(e) => setViewMode(e.target.value as ViewMode)}>
                                <option value="week">Heti</option>
                                <option value="month">Havi</option>
                                <option value="year">Éves</option>
                            </select>
                            <button className="pt-todayBtn" onClick={() => setAnchorDate(new Date())}>Ma</button>
                        </div>
                    </div>
                    ...
                    <div className="pt-ganttGrid">
                        ...
                        {days.map((d) => (
                            <div key={d.toISOString()} className={`pt-col ${isWeekend(d) ? "pt-col--weekend" : ""} ${sameDay(d, today) ? "pt-col--today" : ""}`}>
                        ...
                        {filteredSorted.map((w) => {
                            ...
                            // A sáv látható része (levágjuk, ami kilóg a nézetből)
                            const visibleStart = new Date(Math.max(w.start.getTime(), rangeStart.getTime()));
                            const visibleEnd = new Date(Math.min(w.end.getTime(), rangeEnd.getTime()));

                            // Pozíció kiszámítása
                            const startOffset = daysBetweenInclusive(rangeStart, visibleStart) - 1;
                            ...
                            return (
                                <div key={w.munka_id} className="pt-gridRow" onClick={() => setSelectedWork(w)}>
                                    <div />
                    ...
                    <div className="pt-legend">
                        {(Object.keys(filters) as StatusKey[]).map((key) => (
                            <label key={key} className="pt-legendItem">
                                <input type="checkbox" checked={filters[key]} onChange={(e) => setFilters(prev => ({...prev, [key]: e.target.checked}))} />
            ...
            {selectedWork && (
                <WorkDisplay work={selectedWork} onClose={() => setSelectedWork(null)} onSave={handleWorkSave} />
            )}
        </div>
    );
}
